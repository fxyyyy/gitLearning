$(document).ready(function() {
	...
	//begin:显示该标题下的内容
	function toArticleContent(id,title,content,publishTime,answernum,userId){
		url = "modelChoosePractice.html?id="+id+
		 "&userId="+userId+
	...
	function showPractice(msg){
		...
		if (msg.code == 100) {
			...
			//把值拼接到html中
			var list = msg.map.listPracticePage;
			...
			$.each(list, function(index, inform) {
				...
				var informTime = formateDate(inform.starttime);
				...
				title.click(function(){
					toArticleContent(inform.id,inform.title,inform.content,
							informTime,inform.answernum,nameList[index])});
